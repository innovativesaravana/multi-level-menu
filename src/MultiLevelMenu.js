...
const Item = props => {
  ...
  return (
    <div className={`item ${name}`} onClick={() => itemClicked(name)} >
      ...
      {
        (!_.isUndefined(item.subitems)&&<span onMouseEnter={() => displaySubmenu(item.subitems)}>&#x25BA;</span>)
      }
    </div>
  );
};
  ...
  return (
    <div className={`group ${name}`}>
      {_.map(sortedItems, item => {
        return <Item item={item} displaySubmenu={displaySubmenu} itemClicked={itemClicked}/>;
      })}
    </div>
  );
};
...
const MenuContainer = props => {
  ...
  return(
    <div className="container">
      {_.map(data, group => {
        return <Group name="group 3" items={group} displaySubmenu={displaySubmenu} itemClicked={itemClicked}/>;
      })}
    </div>
  )
}
      ...
      <div className="App">
        <div class="test">Selected Option: {this.state.selectedOption}</div>
        {
          _.map(this.state.levelArray, data => {
            return <MenuContainer data={data} displaySubmenu={displaySubmenu} itemClicked={itemClicked}/>
          })
        }
      </div>
    );
